...
* Архитектура процессора для отладочной сборки
  arm64-v8a, если параметр не задан; пустое значение (`DEBUG_ABI=`) - все архитектуры, так собирает CI
  Задаётся:
...
## Автоматическая сборка и выпуск (GitHub Actions)

Описание сборки — `.github/workflows/build.yml`. Debug и release собираются под все архитектуры и подписываются ключом из `appKey`.
